...
.. py:class:: mindspore_xai.explainer.Gradient(network)
    ...
    **输入：**
    ...
    - **ret** (str) - 返回对象的类型。'tensor'表示返回Tensor ，而'image'表示返回PIL.Image.Image的list。默认值：'tensor'。
    - **show** (bool, 可选) - 显示热力图（saliency map)，设置为 `None` 表示自动。默认值：`None` 。

    **输出：**

    Tensor，shape为 :math:`(N, 1, H, W)` 的4D Tensor，热力图。而如果 `ret` 设置为'image'，则输出list[list[PIL.Image.Image]]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.Deconvolution(network)
    ...
    .. note::
        ...
        解析后的 `network` 将通过 `network.set_grad(False)` 和 `network.set_train(False)` 设置为eval模式。如果您想在之后训练 `network` ，请通过相反的方式将其重置为训练模式。在使用 `Deconvolution` 时，网络中的 `ReLU` 必须用 `mindspore.nn.Cell` 类来实现，而不是用 `mindspore.ops.Operations.ReLU` ，否则，将会导致错误结果。
    ...
    **输入：**
    ...
    - **ret** (str) - 返回对象的类型。'tensor'表示返回 Tensor，而'image'表示返回PIL.Image对象。默认值：'tensor'。
    - **show** (bool) - 如果在JupyterLab或Notebook上运行，将自动显示热力图。默认值：`True`。

    **输出：**

    Tensor，shape为 :math:`(N, 1, H, W)` 的 4D Tensor。或如果 `ret` 设置为'image'，则输出list[PIL.Image.Image]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.GuidedBackprop(network)
    ...
    .. note::
        ...
        解析后的 `network` 将通过 `network.set_grad(False)` 和 `network.set_train(False)` 设置为eval模式。如果想在之后训练 `network` ，请通过相反的方式将其重置为训练模式。要使用 `GuidedBackprop` 时，网络中的 `ReLU` 必须用 `mindspore.nn.Cell` 类来实现，而不是用 `mindspore.ops.Operations.ReLU` ，否则，将会导致错误结果。
    ...
    **输入：**
    ...
    - **ret** (str) - 返回对象的类型。'tensor'表示返回Tensor，而'image'表示返回PIL.Image.Image的list。默认值： 'tensor'。
    - **show** (bool, 可选) - 如果在JupyterLab或Notebook上运行，将自动显示热力图。默认值： `None` 。

    **输出：**

    Tensor，shape为 :math:`(N, 1, H, W)` 的4D Tensor，热力图。或如果 `ret` 设置为'image'，则输出list[list[PIL.Image.Image]]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.GradCAM(network, layer="")
    ...
    **参数：**
    ...
    - **network** (Cell) - 要解释的黑盒模型。
    - **layer** (str, 可选) - 生成解释的层名称，为了更好的实践效果，通常选择最后一个卷积层。如果是''，则将在输入层生成解释。默认值：''。
    ...
    **输入：**
    ...
    - **targets** (Tensor, int, tuple, list) - 目标分类，1D/Scalar Tensor、integer，或integer类型的tuple/list。如果是1D Tensor、tuple或list，其长度应为 :math:`N`。
    - **ret** (str) - 返回对象的类型。'tensor'表示返回Tensor，而'image'表示返回PIL.Image.Image的list。默认值：'tensor'。
    - **show** (bool, 可选) - 显示热力图，设置为 `None` 表示自动。默认值： `None` 。

    **输出：**

    Tensor，shape为 :math:`(N, 1, H, W)` 的4D Tensor，热力图。或如果 `ret` 设置为'image'，则输出 list[list[PIL.Image.Image]]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.SHAPGradient(network, features, feature_names=None, class_names=None, num_neighbours=200, max_features=10)
    ...
    **输入：**
    ...
    - **targets** (Tensor, numpy.ndarray, list, int, 可选) - 要解释的目标分类。当 `target` 是integer时，生成该目标的归因图(attribution map)。而当 `targets` 为Tensor、numpy数组或list时，它的shape则是 :math:`(N, L)` ，L是每个样例的标签数量， :math:`(N,)` 或者 :math:`()` 。默认值：0。
    - **show** (bool, 可选) - 显示解释图像，设置为 `None` 表示自动。默认值： `None` 。

    **输出：**

    Tensor，shape为 :math:`(N, L, K)` 的3D Tensor。第一个维度表示输入。第二个维度表示目标。第三个维度表示特征权重。
...
.. py:class:: mindspore_xai.explainer.SHAPKernel(predictor, features, feature_names=None, class_names=None, num_neighbours=5000, max_features=10)
    ...
    **参数：**
    ...
    - **predictor** (Callable) - 要解释的黑盒模型，一个可调用的函数。对于分类模型，它接受shape为 :math:`(N, K)` 的2D 数组/Tensor作为输入，并输出shape为 :math:`(N, L)` 的2D数组/Tensor。对于回归模型，它接受shape为 :math:`(N, K)` 的2D数组/Tensor作为输入并输出shape为 :math:`(N)` 的1D数组/Tensor。
    - **features** (Tensor, numpy.ndarray) - 2D Tensor或 :math:`(N, K)` 的2D numpy数组，N是样本数，而K是特征数。用于集成特征的背景数据集，接受全部或部分的训练数据集。
    - **feature_names** (list, 可选) - 训练数据中的列的名称（string）的list。默认值： `None` 。
    - **class_names** (list, 可选) - 类名的 list，排序根据分类器的类名排序。如果没有，类名将被设为‘0’、‘1’、... 默认值： `None` 。
    ...
    **输入：**
    ...
    - **targets** (Tensor, numpy.ndarray, list, int, 可选) - 要解释的目标分类。当 `targets` 是integer时，生成该目标的归因图。而当 `target` 是一个Tensor、numpy数组或list时，它的shape则是 :math:`(N, L)` ，L是每个样例的标签数量， :math:`(N,)` 或者 :math:`()` 。默认值：0。
    - **show** (bool, 可选) - 显示解释图像，设置为 `None` 表示自动。默认值：`None` 。
...
.. py:class:: mindspore_xai.explainer.Occlusion(network, activation_fn, perturbation_per_eval=32)
    ...
    `Occlusion` 使用滑动窗口将像素替换为一个参考值，例如用一个常数，并参考原输出去计算差异，而这些由扰动像素引起的输出差异，将被设为这些像素的特征重要性。对于多个滑动窗口中涉及的像素，特征重要性将会是多个滑动窗口的平均差异。
    ...
    **参数：**
    ...
    - **network** (Cell) - 要解释的黑盒模型。
    - **activation_fn** (Cell) - 将logits转换为预测概率的激活层。对于单标签分类任务，通常使用 `nn.Softmax` 。而对于多标签分类任务，则较常使用 `nn.Sigmoid` 。用户也可以将自定义的 `activation_fn` 与网络结合，最终输出便是输入的概率。
    ...
    **输入：**
    ...
    - **ret** (str) - 返回对象类型。'tensor'表示返回Tensor，而'image'表示返回PIL.Image.Image的list。默认值：'tensor'。
    - **show** (bool, 可选) - 显示热力图，设置为 `None` 表示自动。默认值： `None` 。
...
.. py:class:: mindspore_xai.explainer.RISE(network, activation_fn, perturbation_per_eval=32)
    ...
    `RISE` 是一种基于扰动的方法，通过在多个随机二进制掩码上采样，来生成归因图。原始图像 :math:`I` 被随机屏蔽，然后输入到黑盒模型以获取预测概率，最后的归因图便是这些随机掩码 :math:`M_i` 的加权和，而权重是目标节点上的相应输出：
    ...
    **输入：**
    ...
    - **targets** (Tensor, int) - 目标分类。当 `targets` 是integer时，生成该目标的归因图。而当 `targets` 是Tensor时，它的shape则是 :math:`(N, L)` ，L是每个样本的标签数量，或 :math:`(N,)` :math:`()`。
    - **ret** (str) - 返回对象类型。'tensor'表示返回Tensor，'image'表示返回PIL.Image.Image list。默认值：'tensor'。
    - **show** (bool, 可选) - 显示热力图，设置为 `None` 表示自动。默认值： `None` 。

    **输出：**

    Tensor，4D Tensor，当目标是shape为 :math:`(N, L)` 的Tensor时，输出的shape便会是 :math:`(N, L, H, W)` ，否则会是 :math:`(N, 1, H, W)` ，热力图。或如果 `ret` 设置为'image'，则输出 list[list[PIL.Image.Image]]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.RISEPlus(ood_net, network, activation_fn, perturbation_per_eval=32)
    ...
    `RISEPlus` 是一种基于扰动的方法，通过在多个随机二进制掩码上采样，来生成归因图。它采用分布外检测器来产生"内值分数"，并用于估计从分布生成样本的概率，然后将内值分数聚合到随机掩码的加权和，而权重是目标节点上的相应输出：
    ...
    **参数：**
    ...
    - **network** (Cell) - 要解释的黑盒模型。
    - **activation_fn** (Cell) - 将logits转换为预测概率的激活层。对于单标签分类任务，通常使用 `nn.Softmax` 。而对于多标签分类任务，则较常使用 `nn.Sigmoid` 。用户还可以将自己自定义的 `activation_fn` 与网络结合，最终输出便是输入的概率。
    - **perturbation_per_eval** (int, 可选) - 在推理扰动样本期间，每次推理的扰动数。在内存容许情况下，通常此数字越大，便越快得到解释。默认值：32。

    **输入：**

    - **inputs** (Tensor) - 要解释的输入数据，shape为 :math:`(N, C, H, W)` 的4D Tensor。
    - **targets** (Tensor, int) - 要解释的目标分类。当 `targets` 是integer时，生成该目标的归因图。而当 `targets` 是Tensor时，它的shape则为 :math:`(N, L)` ，L是每个样本的标签数量，或 :math:`(N,)` :math:`()`。
    - **ret** (str) - 返回对象类型。'tensor'表示返回Tensor，'image'表示返回PIL.Image.Image的list。默认值：'tensor'。
    - **show** (bool, 可选) - 显示热力图，设置为 `None` 表示自动。默认值：`None` 。

    **输出：**

    Tensor，4D Tensor，当目标是shape为 :math:`(N, L)` 的Tensor时，输出的shape便会是 :math:`(N, L, H, W)`，否则会是 :math:`(N, 1, H, W)`，热力图。或如果 `ret` 设置为'image'，则输出list[list[PIL.Image.Image]]，归一化热力图。
...
.. py:class:: mindspore_xai.explainer.LIMETabular(predictor, train_feat_stats, feature_names=None, categorical_features_indexes=None, class_names=None, num_perturbs=5000, max_features=10)
    ...
    解释表格（即矩阵）数据的预测。数值特征会根据训练数据中的平均值和标准差，从 Normal(0,1) 分布中采样，进行均值中心化和缩放的逆运算来进行扰动。而分类特征则会根据训练分布采样进行扰动，当它的值与被解释的实例相同时，将生成一个数值为1的二进制特征。

    **参数：**

    - **predictor** (Callable) - 要解释的黑盒模型，一个可调用的函数。对于分类模型，它接受shape为 :math:`(N, K)` 的2D 数组/Tensor作为输入，并输出shape为 :math:`(N, L)` 的2D数组/Tensor。对于回归模型，它接受shape为 :math:`(N, K)` 的2D 数组/Tensor作为输入并输出shape为 :math:`(N)` 的1D数组/Tensor。
    - **train_feat_stats** (dict) - 具有训练数据统计详细信息的dict对象。统计信息可以使用静态方法 `LIMETabular.to_feat_stats(training_data)` 生成。
    ...
    **输入：**
    ...
    - **targets** (Tensor, numpy.ndarray, list, int, 可选) - 要解释的目标分类。当 `targets` 是integer时，生成该目标的归因图。而当 `targets` 是Tensor、numpy数组或list时，它的shape则是 :math:`(N, L)`，L是每个样例的标签数量， :math:`(N,)`或者 :math:`()`。对于回归模型，此参数将被忽略。默认值：0。
    - **show** (bool, 可选) - 显示解释图像，设置为 `None` 表示自动。默认值： `None` 。

    **输出：**

    list[list[list[(str, float)]]]，一个tuple类的3D list。第一个维度表示输入。第二个维度表示目标。第三个维度表示特征。tuple代表特征的描述和权重。
