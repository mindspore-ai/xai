...
def argmax(inputs: Tensor, axis: int = -1, keep_dims: bool = False) -> Tensor:
    ...
    return Tensor(outputs, mindspore.int32)
...
def argsort(inputs: Tensor, axis: int = -1, descending: bool = False) -> Tensor:
    ...
    indices = Tensor(indices_np, dtype=mindspore.int32)
    return indices


def assign(inputs: Tensor, idx: _IdxType, value: Tensor) -> Tensor:
...
def maximum(inputs: Tensor, axis: _AxisType = (), keep_dims: bool = False) -> Tensor:
...
def minimum(inputs: Tensor, axis: _AxisType = (), keep_dims: bool = False) -> Tensor:
...
def mean(inputs: Tensor, axis: _AxisType = (), keep_dims: bool = False) -> Tensor:
...
def sort(inputs: Tensor, axis: _AxisType = -1, descending: bool = False) -> Tensor:
...
def squeeze(inputs: Tensor, axis: _AxisType = ()):
...
def reshape(inputs: Tensor, shape: _ShapeType) -> Tensor:
...
def zeros(shape: _ShapeType, dtype: mindspore.dtype = None) -> Tensor:
    """Return a new array of given shape and type, filled with zeros."""
    outputs = np.zeros(shape)
    return Tensor(outputs, dtype=dtype)
...
def zeros_like(inputs: Tensor, dtype: mindspore.dtype = None) -> Tensor:
    ...
    outputs = Tensor(outputs_np, dtype)
    return outputs


def random(shape: _ShapeType, dtype: mindspore.dtype = None) -> Tensor:
...
def randint(low: int, high: int, shape: _ShapeType, dtype: mindspore.dtype = mindspore.int8) -> Tensor:
    """Return random integers from `low` (inclusive) to `high` (exclusive)."""
    outputs_np = np.random.randint(low, high, size=shape)
    outputs = Tensor(outputs_np, dtype=dtype)
    return outputs
...
def summation(inputs: Tensor, axis: _AxisType = (), keep_dims: bool = False) -> Tensor:
