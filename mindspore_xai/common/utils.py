...
def abs_max(gradients):
    ...
    Args:
        gradients (Tensor): Gradients which will be transformed to saliency map.
...
def retrieve_layer(model: _module_type, target_layer: Union[str, _module_type] = ''):
    """
    ...
    if isinstance(target_layer, _module_type):
...
class ForwardProbe:
    ...
    def __init__(self, target_layer: _module_type):
...
def calc_auc(x: _ArrayType) -> _ArrayType:
...
def rank_pixels(inputs: _ArrayType, descending: bool = True) -> _ArrayType:
    """
...
def resize(inputs: Tensor, size: Tuple[int, int], mode: str) -> Tensor:
    """
